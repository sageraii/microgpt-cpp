/*
 * export_weights.cpp — 순수 C++17로 구현한 최소 GPT (시각화 데이터 내보내기 버전)
 *
 ...
 *
 * 수정 사항: 학습 과정의 가중치, 어텐션, 손실 등을 data.js로 내보내어
 * 웹 기반 시각화에서 사용할 수 있도록 합니다.
 *
 * 컴파일: g++ -std=c++17 -O2 -o export_weights export_weights.cpp
 * 실행:   ./export_weights
 */
...
// ============================================================================
// 어텐션 캡처: 시각화를 위해 어텐션 가중치를 기록하는 구조체
// ============================================================================

struct AttnSnapshot {
    // head_weights[head][query_pos] = vector of attention weight values
    // 각 헤드별, 각 쿼리 위치별 어텐션 가중치 벡터
    std::vector<std::vector<std::vector<double>>> head_weights;
    void init() { head_weights.resize(N_HEAD); }
};

// ============================================================================
// 학습 스냅샷: 특정 학습 단계의 전체 상태를 기록
// ============================================================================
...
//
// attn_out: nullptr이 아니면 어텐션 가중치를 기록 (시각화용)
// ============================================================================
    ...
    for (int li = 0; li < N_LAYER; li++) {
        ...
        for (int h = 0; h < N_HEAD; h++) {
            ...
            if (attn_out) {
                ...
                aw.reserve(attn_weights.size());
                for (auto* w : attn_weights) aw.push_back(w->data);
                attn_out->head_weights[h].push_back(aw);
            }
...
// ============================================================================
// data.js 내보내기를 위한 헬퍼 함수들
// ============================================================================

// 가중치 행렬을 JavaScript 2D 배열 형태로 출력
void write_matrix(FILE* f, const Mat& m) {
    fprintf(f, "[");
    for (size_t i = 0; i < m.size(); i++) {
        fprintf(f, "[");
        for (size_t j = 0; j < m[i].size(); j++) {
            fprintf(f, "%.6f", m[i][j]->data);
            if (j + 1 < m[i].size()) fprintf(f, ",");
        }
        fprintf(f, "]");
        if (i + 1 < m.size()) fprintf(f, ",");
    }
    fprintf(f, "]");
}

// ============================================================================
// 메인: 데이터 로딩, 학습, 추론, data.js 내보내기
// ============================================================================
...
int main() {
    ...
    // --- 시각화를 위한 데이터 수집 설정 ---
    // 특정 학습 단계에서 스냅샷을 저장하여 시각화에 활용
    std::set<int> snapshot_steps = {0, 10, 50, 100, 200, 500, 999};
    std::vector<double> loss_history;        // 매 스텝의 손실 기록
    std::vector<double> grad_norm_history;   // 매 스텝의 기울기 노름 기록
    std::vector<TrainingSnapshot> snapshots; // 선택된 스텝의 전체 상태 스냅샷
    ...
    for (int step = 0; step < num_steps; step++) {
        ...
        // 스냅샷 스텝인 경우 어텐션 캡처 준비
        bool is_snapshot = snapshot_steps.count(step) > 0;
        AttnSnapshot attn;
        if (is_snapshot) attn.init();
        ...
        // 매 스텝 손실 기록
        loss_history.push_back(loss->data);

        // 역전파: 모든 파라미터에 대한 기울기를 계산
        backward(loss);

        // 기울기 노름 계산 (역전파 후, Adam 업데이트 전)
        double gn = 0;
        for (auto* p : params) gn += p->grad * p->grad;
        grad_norm_history.push_back(std::sqrt(gn));

        // 스냅샷 스텝이면 전체 상태를 저장
        if (is_snapshot) {
            snapshots.push_back(TrainingSnapshot{step, doc, tokens, loss->data, attn});
        }
    ...
    // ============================================================================
    // data.js 내보내기: 학습된 모델의 가중치, 학습 기록, 어텐션 스냅샷을 저장
    // ============================================================================
    std::cout << "--- exporting data.js ---\n";

    FILE* f = fopen("data.js", "w");
    if (!f) {
        std::cerr << "data.js 파일을 열 수 없습니다.\n";
        return 1;
    }

    // --- config 섹션 ---
    fprintf(f, "const MODEL_DATA = {\n");
    fprintf(f, "  config: { vocab_size: %d, n_embd: %d, block_size: %d, n_head: %d, head_dim: %d, n_layer: %d, num_params: %d },\n",
            vocab_size, N_EMBD, BLOCK_SIZE, N_HEAD, HEAD_DIM, N_LAYER, (int)params.size());

    // --- vocab 섹션 ---
    fprintf(f, "  vocab: [");
    for (int i = 0; i < (int)uchars.size(); i++) {
        fprintf(f, "\"%c\"", uchars[i]);
        fprintf(f, ",");
    }
    fprintf(f, "\"<BOS>\"");
    fprintf(f, "],\n");
    ...
    // --- weights 섹션 ---
    // 고정된 키 순서로 가중치 행렬을 내보낸다
    fprintf(f, "  weights: {\n");

    // 전역 가중치: wte, wpe, lm_head
    fprintf(f, "    wte: ");
    write_matrix(f, state_dict["wte"]);
    fprintf(f, ",\n");

    fprintf(f, "    wpe: ");
    write_matrix(f, state_dict["wpe"]);
    fprintf(f, ",\n");

    fprintf(f, "    lm_head: ");
    write_matrix(f, state_dict["lm_head"]);
    fprintf(f, ",\n");
    ...
    for (int li = 0; li < N_LAYER; li++) {
        std::string pfx = "layer" + std::to_string(li);
        for (size_t si = 0; si < layer_suffixes.size(); si++) {
            std::string key = pfx + layer_suffixes[si];
            fprintf(f, "    \"%s\": ", key.c_str());
            write_matrix(f, state_dict[key]);
            // 마지막 항목이 아니면 쉼표
            if (li < N_LAYER - 1 || si + 1 < layer_suffixes.size()) {
                fprintf(f, ",\n");
            } else {
                fprintf(f, "\n");
            }
    ...
    // loss_history 배열
    fprintf(f, "    loss_history: [");
    for (size_t i = 0; i < loss_history.size(); i++) {
        fprintf(f, "%.6f", loss_history[i]);
        if (i + 1 < loss_history.size()) fprintf(f, ",");
    }
    fprintf(f, "],\n");

    // grad_norm_history 배열
    fprintf(f, "    grad_norm_history: [");
    for (size_t i = 0; i < grad_norm_history.size(); i++) {
        fprintf(f, "%.6f", grad_norm_history[i]);
        if (i + 1 < grad_norm_history.size()) fprintf(f, ",");
    }
    fprintf(f, "],\n");
    ...
    for (size_t si = 0; si < snapshots.size(); si++) {
        const auto& snap = snapshots[si];
        fprintf(f, "      { step: %d, doc: \"%s\", tokens: [", snap.step, snap.doc.c_str());
        for (size_t ti = 0; ti < snap.tokens.size(); ti++) {
            fprintf(f, "%d", snap.tokens[ti]);
            if (ti + 1 < snap.tokens.size()) fprintf(f, ",");
        }
        fprintf(f, "], loss: %.6f,\n", snap.loss);

        // attention: 3D array [head][query_pos][key_pos]
        fprintf(f, "        attention: [");
        for (int h = 0; h < N_HEAD; h++) {
            fprintf(f, "[");
            const auto& head = snap.attention.head_weights[h];
            for (size_t qi = 0; qi < head.size(); qi++) {
                fprintf(f, "[");
                for (size_t ki = 0; ki < head[qi].size(); ki++) {
                    fprintf(f, "%.6f", head[qi][ki]);
                    if (ki + 1 < head[qi].size()) fprintf(f, ",");
                }
                fprintf(f, "]");
                if (qi + 1 < head.size()) fprintf(f, ",");
            }
            fprintf(f, "]");
            if (h + 1 < N_HEAD) fprintf(f, ",");
        }
        fprintf(f, "] }");
        if (si + 1 < snapshots.size()) fprintf(f, ",");
        fprintf(f, "\n");
    }
    fprintf(f, "    ]\n");
    ...
    fclose(f);
    std::cout << "data.js exported successfully.\n";
